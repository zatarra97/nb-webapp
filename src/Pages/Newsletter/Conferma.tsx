...
const Conferma = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${BACKEND_URL}/confirm?token=${encodeURIComponent(token)}`)
      .then(() => setStatus('ok'))
    ...
    <PublicLayout>
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <div className="text-center max-w-sm">
          ...
          {status === 'ok' && (
            <>
              <div className="w-14 h-14 rounded-full bg-gray-900 text-white flex items-center justify-center mx-auto mb-6">
                <i className="fa-solid fa-check text-xl" />
              </div>
              <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('newsletter.confirmed_title')}</h1>
              <p className="text-gray-500 text-sm mb-6">{t('newsletter.confirmed_body')}</p>
